...
contract StakeUIHelper is StakeUIHelperI {
    ...
    address public immutable SMARTLEND;
    IStakedToken public immutable STAKED_SMARTLEND;
    ...
    address constant MOCK_USD_ADDRESS =
    ...
    function _getUserAndGeneralStakedAssetData(
        ...
        GeneralStakeUIData memory generalStakeData = _getGeneralStakedAssetData(
            stakeToken
        );
        ...
        if (user != address(0)) {
            ...
            data.underlyingTokenUserBalance = userStakeData
    ...
    function _getGeneralStakedAssetData(
        ...
        data.rewardTokenPriceEth = PRICE_ORACLE.getAssetPrice(SMARTLEND);
    ...
    function getStkSmartLendData(
        address user
    ) public view override returns (AssetUIData memory) {
        AssetUIData memory data = _getUserAndGeneralStakedAssetData(
            STAKED_SMARTLEND,
            SMARTLEND,
            user,
            false
    ...
    function getStkGeneralSmartLendData()
        public
        view
        override
        returns (GeneralStakeUIData memory)
    {
        GeneralStakeUIData memory data = _getGeneralStakedAssetData(
            STAKED_SMARTLEND
        );
    ...
    function getStkUserSmartLendData(
        address user
    ) public view override returns (UserStakeUIData memory) {
        UserStakeUIData memory data = _getUserStakedAssetData(
            STAKED_SMARTLEND,
            SMARTLEND,
            user,
            false
    ...
    function getUserUIData(
        ...
        view
        override
        returns (AssetUIData memory, AssetUIData memory emptyData, uint256)
    {
        return (
            getStkSmartLendData(user),
            emptyData,
