...
// File @smartlend/smartlend-token/contracts/open-zeppelin/Context.sol@v1.0.4
...
// File @smartlend/smartlend-token/contracts/open-zeppelin/ERC20.sol@v1.0.4
...
// File contracts/interfaces/IStakedSmartLend.sol
 ...
 *
 * @author SmartLend, inspired by the OpenZeppelin Initializable contract
 */
...
// File contracts/interfaces/ISmartLendDistributionManager.sol

interface ISmartLendDistributionManager {
    function configureAssets(
        DistributionTypes.AssetConfigInput[] calldata assetsConfigInput
    ) external;
}

// File contracts/stake/SmartLendDistributionManager.sol

/**
 * @title SmartLendDistributionManager
 * @notice Accounting contract to manage multiple staking distributions
 * @author SmartLend
 **/
contract SmartLendDistributionManager is ISmartLendDistributionManager {
    using SafeMath for uint256;
     ...
     * @param underlyingAsset The address used as key in the distribution, for example sSMARTLEND or the aTokens addresses on SmartLend
...
// File @smartlend/smartlend-token/contracts/interfaces/IGovernancePowerDelegationToken.sol@v1.0.4
...
// File @smartlend/smartlend-token/contracts/token/base/GovernancePowerDelegationERC20.sol@v1.0.4

/**
 * @notice implementation of the SMARTLEND token contract
 * @author SmartLend
 */
    ...
    IGovernancePowerDelegationToken
{
     ...
     * In this initial implementation with no SMARTLEND minting, simply returns the current supply
     * A snapshots mapping will need to be added in case a mint function is added to the SMARTLEND token in the future
     **/
    function totalSupplyAt(
        uint256 blockNumber
    ) external view override returns (uint256) {
     ...
     * the SMARTLEND token and StakeToken already include a mapping for the snapshots, so we require contracts
...
/**
 * @title StakedToken
 * @notice Contract to stake SmartLend token, tokenize the position and get rewards, inheriting from a distribution manager contract
 * @author SmartLend
 **/
    ...
    SmartLendDistributionManager
{
    ...
    constructor(
        ...
        public
        ERC20(name, symbol)
        SmartLendDistributionManager(emissionManager, distributionDuration)
    {
        ...
        _smartlendGovernance = ITransferHook(governance);
    ...
    function _beforeTokenTransfer(
        ...
        // caching the smartlend governance address to avoid multiple state loads
        ITransferHook smartlendGovernance = _smartlendGovernance;
        if (smartlendGovernance != ITransferHook(0)) {
            smartlendGovernance.onTransfer(from, to, amount);
        }
    }
